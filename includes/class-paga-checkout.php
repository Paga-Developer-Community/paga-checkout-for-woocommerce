...
class WC_Paga_Checkout extends WC_Payment_Gateway
{
    ...
    public function __construct()
    {
        ...
        $this->method_description=sprintf(
            __('Paga Checkout provides an easy-to-integrate payment collection tool for any online merchant. It supports funding sources from Cards, Bank accounts and Paga wallet.<a href="%1$s" target="_blank">Sign up</a> for a Paga account, and <a href="%2$s" target="_blank">get your paga-checkout credentials</a>.', 'paga-checkout'),
        ...
        add_action('admin_notices', array($this, 'chargeUrl_notice'));
    ...
    public function chargeUrl_notice()
    {
        if ($this ->enabled == 'no') {
            return;
        }
        ...
        if (!($this->charge_url)) {
            echo '<div class="notice notice-error" </p>' .sprintf(__('Please enter your Url to redirect after successful transaction <a href="%s">here</a> to be able to use Paga Checkout WooCommerce plugin.', 'paga-checkout'), admin_url('admin.php?page=wc-settings&tab=checkout&section=paga-checkout')) .'</p></div>';
            return;
        }
    }
    ...
    public function init_form_fields()
    {
        $form_fields = array(
              ...
              'charge_url'                     => array(
                'title'       => __('Data Charge url', 'paga-checkout'),
                'type'        => 'text',
                'description' => __('Url to redirect after successful transaction', 'paga-checkout'),
                'default'     => '',
    ...
    public function generate_paga_checkout_widget()
    {
        ...
        $paga_checkout_params = array(
                ...
                'charge_url'=> isset($this->charge_url) && ($this->charge_url !== '') ? esc_url_raw($this->charge_url): wc_get_page_permalink('shop'),
        ...
        if ($this->testmode) {
            $paga_checkout_params['checkout']  = esc_url_raw('https://qa1.mypaga.com/checkout/');
